...
export default function PlayerBar() {
  const {
    ...
    cycleRepeat, setVolume, toggleMute, seekTo, toggleFavorite,
  } = usePlayerStore()
  ...
  // Click on the track without dragging fires onChange and onPointerUp almost instantly.
  ...
  return (
    <div className="player-bar">
      ...
      <div className="player-bar__song">
        ...
        {currentSong && (
          <button
            className={`player-bar__fav${currentSong.isFavorite ? ' active' : ''}`}
            onClick={() => window.api.library.toggleFavorite(currentSong.id)}
          >
            <HeartIcon filled={!!currentSong.isFavorite} />
          </button>
        )}
      </div>
